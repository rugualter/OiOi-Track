...
def importer(file, user, mode):
    ...
    csv_importer = oioitrackImporter(file, user, mode)
    return csv_importer.import_data()


class oioitrackImporter:
    ...
    def __init__(self, file, user, mode):
        ...
        logger.info(
            "Initialized OiOi-Track CSV importer for user %s with mode %s",
            user.username,
            mode,
        )
    ...
    def import_data(self):
        ...
        for row in reader:
            try:
                ...
                error_msg = (
                    _("Error processing entry with ID %(media_id)s "
    ...
    def _handle_missing_metadata(self, row, media_type, season_number, episode_number):
        ...
        msg = _("Missing metadata for: %(row)s") % {
            "row": row,
        }
